...
for i in files_simp:
    files_i = [x for x in files if "%s_"%i in x] + [x for x in files if "%s."%i in x]
    use_date = ""
    if ("20" in "".join(files_i)): # Disregard files that do not have dates
        
        files_i_dated = [x for x in files_i if "20" in x]
        # Get date list & find the most recent date
        files_i_dates = [x.split(".")[-2].split("_")[-1].replace("new", "") for x in files_i_dated]
