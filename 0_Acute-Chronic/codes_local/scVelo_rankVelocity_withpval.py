...
from scvelo.utils import strings_to_categoricals, vcorrcoef
...
def rank_velocity_genes(data, vkey='velocity', n_genes=100, groupby=None, match_with=None, resolution=None,
    ...
    rankings_gene_names, rankings_gene_scores, rankings_gene_pvals, indices = [], [], [], []
    for i in range(n_groups):
        ...
        ##### Changed here to get p values & use stats package for t-test
        #scores = (mean[i] - mean_rest) / np.sqrt(var[i] / sizes[i] + var_rest / size_rest)
        #scores = np.nan_to_num(scores)

        with np.errstate(invalid="ignore"):
            scores, pvals = stats.ttest_ind_from_stats(
                mean1=mean[i], std1=np.sqrt(var[i]), nobs1=sizes[i],
                mean2=mean_rest,  std2=np.sqrt(var_rest),  nobs2=size_rest,
        ...
        rankings_gene_pvals.append(pvals[idx])
    ...
    rankings_gene_pvals = np.array([list(n) for n in rankings_gene_pvals])
    ...
    adata.uns[key] = \
        {'names': np.rec.fromarrays([n for n in rankings_gene_names], dtype=[(str(rn), 'U50') for rn in groups]),
         ...
         'pvals': np.rec.fromarrays([n.round(4) for n in rankings_gene_pvals], dtype=[(str(rn), 'float32') for rn in groups]),
