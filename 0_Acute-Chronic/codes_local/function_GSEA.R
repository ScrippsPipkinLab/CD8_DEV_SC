...
filename_simp <- function(in_filename) {
  ...
  cpx <- gsub("---all_GSEA_20200205", "", cpx)
  cpx <- gsub("---all_GSEA", "", cpx)
  cpx <- gsub("---GSE88987-Exp276_slt_GSEA", "", cpx)
  cpx <- gsub("---best_GSEA", "", cpx)
  cpx <- gsub("---JJM_GSEA", "", cpx)
  cpx <- gsub("---Yao_Tox_sc_GSEA", "", cpx)
  cpx <- gsub("louvain_", "C", cpx)
  cpx <- gsub("---Combined_GEO_GSEA_signatures", "", cpx)
  return(cpx)
}
...
GSEA_sum <- function(file_list, outname, wid, hei, comp_use_order, comp_order_vec, path_use_order, path_order_vec) {
  ...
  for (filex in file_list) {
    ...
    filex.vec <-unlist(strsplit(filex, "/"))
    filex.name.simp <- tail(filex.vec, n=1)
